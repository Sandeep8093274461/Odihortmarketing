app.controller('myADHChangePasswordCtrl', function ($scope, $http, $filter) {
            ...
            if (strongRegex.test($scope.txtNewPassword)) {
                if (sha256($scope.txtOldPassword) !== obj.NewPassword) {
                    if (obj.NewPassword === obj.ConfirmPassword) {
                        $http.post('http://localhost:3000/adh/changePassword', { data: obj }, { credentials: 'same-origin', headers: { 'CSRF-Token': token } }).then(function success(response) {
